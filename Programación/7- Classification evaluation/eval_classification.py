...
def eval_classification(params):
    ...
    prediction_file = pd.read_csv(os.path.join(params['root'],params['database'],'val','Classify.txt'), sep='\t', header = 0)
    prediction_labels = prediction_file['ClassID'].tolist()
    prediction_names = prediction_file['ImageID'].tolist()
